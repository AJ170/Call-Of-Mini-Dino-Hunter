...
public class iServerDataManager : MonoBehaviour
{
	...
	public void SendFetchSaveData(string userid, OnSuccessFetch successfunc, OnFailed failedfunc)
	{
		...
		HttpClient.Instance().SendRequest(m_sServerName, "userHandler.loadProfile", userid, "_ServerDataManager", "iServerDataManager", "OnFetchSaveDataResult", null);*/
	}

	protected void OnFetchSaveDataResult(int taskId, int result, string server, string action, string response, string param)
	{
		LogMgr.Log("OnFetchSaveDataResult " + result + " " + action + " " + response + " " + param);
		if (result != 0)
		{
			return;
		}
	...
	{
		LogMgr.Log("OnUploadSaveDataResult " + result + " " + action + " " + response);
		if (result != 0)
		{
			return;
		}
		...
		try
		{
			...
			if (text == "0")
			{
				...
				LogMgr.Log("upload sucess random = " + str2);
