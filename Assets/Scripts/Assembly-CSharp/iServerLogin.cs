...
public class iServerLogin : MonoBehaviour
{
	...
	{
		LogMgr.Log(game + " " + version + " " + deviceId + " " + gamecenterId);
	...
	{
		LogMgr.Log("OnLoginResult " + result + " " + action + " " + response + " " + param);
		if (result != 0)
		{
			return;
		}
		...
		try
		{
			...
			if (text == "0")
			{
				...
				LogMgr.Log("serverId = " + @string);
				LogMgr.Log("serverUrl = " + string2);
				LogMgr.Log("deviceId = " + string3);
				LogMgr.Log("userId = " + string4);
				LogMgr.Log("gamecenterId = " + string5);
