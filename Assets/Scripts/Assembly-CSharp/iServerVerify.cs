...
public class iServerVerify : MonoBehaviour
{
				...
				else if (item2.Name == "popularize")
				{
					...
					foreach (XmlNode item11 in item2)
					{
						if (!(item11.Name != "node") && GetAttribute(item11, "type", ref value))
						{
							...
							LogMgr.Log(cPopularizeInfo.nType + " " + cPopularizeInfo.nID + " " + cPopularizeInfo.sBundleID);
	...
	protected IEnumerator Connect()
	{
        ...
        document.LoadXml(ConfigSpoof.GetConfig("serverspoof"));
        m_ServerConfigInfo.LoadData(document);

		yield return ServerX.CheckVersion();

        switch (ServerX.state)
		{
			case ServerX.ServerCheckState.Invalid:
                m_PingState = kPingState.Success; 
				if (m_OnFailed != null) m_OnFailed();
                yield break;
			case ServerX.ServerCheckState.Valid:
				yield return ServerX.GetServerTime();

                WWW www = new WWW(ServerX.tnetConfigUrl);

                yield return www;

                if (string.IsNullOrEmpty(www.error))
                {
                    var list = ServerX.Parse(www.text);

                    foreach (var pair in list)
                    {
                        var info = new CServerConfigInfo.CGameServerInfo()
                        {
                            sUrl = pair.Key,
                            nPort = pair.Value
                        };

                        m_ServerConfigInfo.m_ltGameServerInfo.Add(info);
                    }
                }
                break;
			case ServerX.ServerCheckState.Unreachable:
                iLoginManager.GetInstance().OnGetServerTime_S((DateTime.UtcNow - new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc)).TotalMilliseconds);

                Debug.Log("ServerX.ServerCheckState.Unreachable");
                break;
		}
		...
		if (www.error != null)
		{
			LogMgr.Log("net error " + www.error);
			m_PingState = kPingState.Fail;
			if (m_OnNetError != null)
			{
				m_OnNetError();
		...
		{
			LogMgr.Log("version error " + m_ServerConfigInfo.m_sVersion);
			m_PingState = kPingState.Fail;
			if (m_OnFailed != null)
			{
				m_OnFailed();
		...
		{
			LogMgr.Log("serverconfig successed ");
			m_PingState = kPingState.Success;
			if (m_OnSuccess != null)
			{
				m_OnSuccess();
	...
	{
		LogMgr.Log("LoadServerData " + input);
		...
		try
		{
			...
			xmlDocument.LoadXml(ConfigSpoof.GetConfig("serverspoof"));
			m_ServerConfigInfo.LoadData(xmlDocument);
		}
		catch (Exception ex)
		{
			LogMgr.LogError("LoadServerData Error " + ex);
		}
	}
