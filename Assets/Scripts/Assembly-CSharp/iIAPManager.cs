...
public class iIAPManager : MonoBehaviour
{
	...
	private static void AwakeChartBoost()
	{
		...
		LogMgr.Log("ChartBoostAndroid init");
	}
	...
	{
		LogMgr.Log("billingSupportedEvent");
	}

	private void billingNotSupportedEvent(string error)
	{
		LogMgr.Log("billingNotSupportedEvent: " + error);
	}
	...
	private void queryInventorySucceededEvent(List<GooglePurchase> purchases, List<GoogleSkuInfo> skus)
	{
		if (itemOwned)
		{
			LogMgr.Log("consumeProduct : " + m_CurBuyingIapId);
			GoogleIAB.consumeProduct(m_CurBuyingIapId);
			itemOwned = false;
		}
		LogMgr.Log(string.Format("queryInventorySucceededEvent. total purchases: {0}, total skus: {1}", purchases.Count, skus.Count));
	...
	private void queryInventoryFailedEvent(string error)
	{
		...
		LogMgr.Log("queryInventoryFailedEvent: " + error);
	}

	private void purchaseCompleteAwaitingVerificationEvent(string purchaseData, string signature)
	{
		LogMgr.Log("purchaseCompleteAwaitingVerificationEvent. purchaseData: " + purchaseData + ", signature: " + signature);
	}

	private void purchaseSucceededEvent(GooglePurchase purchase)
	{
		LogMgr.Log("google purchaseSucceededEvent: " + purchase.ToString());
		GoogleIAB.consumeProduct(purchase.productId);
	}

	private void consumePurchaseSucceededEvent(GooglePurchase purchase)
	{
		LogMgr.Log("google consumePurchaseSucceededEvent");
	...
	public void GoogleOnPurchaseSuccess(GooglePurchase purchase)
	{
		...
		{
			LogMgr.Log("GoogleOnPurchaseSuccess purchase : " + purchase.ToString());
	...
	{
		LogMgr.Log("consumePurchaseFailedEvent: " + error);
	...
	{
		LogMgr.Log("---- purchaseFailedEvent: " + reason + " | iapid : " + m_CurBuyingIapId);
		...
		LogMgr.LogWarning("reason id : " + text);
	}
	...
	public void Purchase(string sIAPKey, OnIAPPurchaseSuccess onsuccess, OnEvent onfailed, OnEvent oncancel, OnEvent onneterror)
	{
		...
		{
			LogMgr.Log("purchase iap " + sIAPKey);
