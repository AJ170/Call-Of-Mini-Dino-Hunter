...
public class iGameApp
{
...
#else
        MyUtils.SimulatePlatform = PlatformEnum.Windows;
#endif
	...
	public bool CheckAchieveReward()
	{
		...
		foreach (CAchievementData value in achiDataData.Values)
		{
			...
			{
				LogMgr.Log(value.nID + " has " + achiStar + " stars");
				if (!value.IsGotReward(achiStar - 1))
				{
					LogMgr.Log("You has not take reward " + (achiStar - 1));
					return true;
				}
			}
		}
	...
	public string PackSaveData()
	{
		...
		{
			LogMgr.LogError("pack data exception");
			return string.Empty;
		}
	}
	...
	public bool IsTutorialStage(int nStageID)
	{
		...
		LogMgr.Log(string.Concat("newhelpstate ", nTutorialVillageState, " ", nStageID));
