...
public class TNetManager : MonoBehaviour
{
	...
	{
		LogMgr.Log("SetCustomMsgFunc " + func.ToString());
	...
	{
		LogMgr.Log("OnHearBeat servertime = " + tEvent.data["serverTime"]);
	}
	...
	protected void OnRoomList(TNetEventData tEvent)
	{
		...
		LogMgr.Log("OnRoomList page = " + num + " pagenum = " + num2 + "  listtype = " + listType);
	...
	protected void OnJoinRoom(TNetEventData tEvent)
	{
		...
		if (result != 0)
		{
			LogMgr.LogError("OnJoinRoom failed result = " + result);
			CGameNetManager.GetInstance().SearchRoom();
			return;
		}
		TNetRoom tNetRoom = (TNetRoom)tEvent.data["room"];
		LogMgr.Log("OnJoinRoom " + tNetRoom.Name);
	...
	{
		TNetRoom tNetRoom = (TNetRoom)tEvent.data["room"];
		LogMgr.Log("OnLeaveRoom " + tNetRoom.Name);
	...
	protected void OnRoomVarChange(TNetEventData tEvent)
	{
		...
		LogMgr.Log(string.Concat("OnRoomVarChange ", tNetUser.Id, " ", tNetUser.Name, " ", tNetRoomVarType, " ", @int, " ", @bool));
	}
	...
	public void Connect(string sIP, int nPort)
	{
		...
		{
			LogMgr.Log("Connect " + sIP + " port " + nPort);
