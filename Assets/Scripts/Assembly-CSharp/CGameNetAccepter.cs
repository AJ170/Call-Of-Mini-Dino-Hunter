...
public class CGameNetAccepter
{
    ...
    protected void OnMsg_PLAYER_MOVE(TNetUser sender, SFSObject data)
    {
        ...
        if (netUserInfo != null)
        {
            ...
            {
                nmsg_player_move nmsg = new nmsg_player_move();
                nmsg.UnPack(data);

                // Instead of using whole path, pick the last position in the path as target
                if (nmsg.m_ltPath.Count > 0)
                {
                    Vector3 newTarget = nmsg.m_ltPath[nmsg.m_ltPath.Count - 1];
                    player.UpdateTargetPosition(newTarget);
                }
            }
        }
    }
    ...
    protected void OnMsg_BATTLE_RESULT_PLAYERREWARDS(TNetUser sender, SFSObject data)
    {
        if (m_GameScene != null && !sender.IsItMe)
        {
            ...
            if (netUserInfo != null)
            {
                ...
                netUserInfo.m_nMVPId = nmsg_battle_result_playerrewards2.m_nMVPId;
