...
public class iGameSceneBase
{
    ...
    public void InitializeGameLevel(int nLevel)
    {
        ...
        if (m_curGameLevelInfo.ltMonsterNumLimit.Count > 0)
        {
            foreach (MonsterNumLimitInfo item in m_curGameLevelInfo.ltMonsterNumLimit)
            {
                ...
                LogMgr.Log(monsterNumInfo.nLimitType + " " + monsterNumInfo.nMax);
        ...
        if (btnPurchase == null)
        {
            GameObject purchasePos = GameObject.Find("purchasepos");
            if (purchasePos != null)
            {
                Transform btn = purchasePos.transform.Find("btnPurchase");
                if (btn != null)
                    btnPurchase = btn.gameObject;
            }
        }
    }
    ...
    public virtual void StartGame()
    {
        ...
        LogMgr.Log("user level = " + character.nLevel);
        for (int i = 0; i < 3; i++)
        {
            int nID = 0;
            int nLevel = 0;
        ...
        if (m_GameUI != null)
        {
            ...
            {
                LogMgr.Log(dataCenter.NickName);
        ...
        if (m_bMissionSuccess)
        {
            ...
            if (task != null)
            {
                ...
                if (taskInfo != null && taskInfo.nType == 2 && m_GameState.LastKillBoss != -1)
                {
                    ...
                    LogMgr.Log(m_GameState.LastKillBoss + " " + mob);
                    if (mob != null)
                    {
    ...
    public virtual void GameOver(bool bSuccess)
    {
        ...
        if (CGameNetManager.GetInstance().IsConnected())
        {
            ...
            if (netUserInfo != null)
            {
                ...
                netUserInfo.m_nMVPId = m_GameState.m_nMVPId;

                CGameNetSender.GetInstance().SendMsg_BATTLE_RESULT_PLAYERREWARDS(netUserInfo.m_nTitle, netUserInfo.m_nCombatRatings, m_GameState.GainCrystalInGame, num2, nBeadmireCount, m_User.Level, m_User.EXP, m_DataCenter.HunterLvl, m_DataCenter.HunterExp, netUserInfo.m_nMVPId);
            }
            m_User.SetFire(false);
            m_User.MoveStop();
            if (MyUtils.isWindows)
    ...
    protected virtual void UpdateStatus_Gaming(float deltaTime)
    {
        ...
        if (m_TaskManager != null)
        {
            ...
            if (CGameNetManager.GetInstance().IsRoomMaster())
            {
                ...
                {
                    LogMgr.Log("over " + m_TaskManager.isAllCompleted + " " + m_TaskManager.isFailed);
                    FinishRevive(false);
    ...
    public void ShowPauseUIOnly(bool show)
    {
        m_GameUI.ShowPauseUI(show);
    }
    ...
    {
        // Disable IAP completely
        Debug.Log("IAP panel disabled — skipping purchase flow.");
        return;
    ...
    public void StartRevive(float fTime)
    {
        ...
        SetCursorLock(false);
        m_fWaitingReviveTime = m_bMutiplyGame ? 0f : fTime;
        m_fWaitingReviveTimeCount = 0f;
    ...
    public void ShowGameOverUI_Win_Mutiply()
    {
        ...
        CGameNetManager.CNetUserInfo tempUser = null;
        ...
        {
            var userInfo = array[k];
            if (userInfo == null) continue;

            gyUIPlayerRewards rewardsUI = m_GameUI.GetPanelMissionSuccessMutiply_PlayerRewards(k);
            if (rewardsUI == null) break;

            userInfo.m_nResultIndex = k;

            if (rewardsUI.m_PlayerName != null)
                rewardsUI.m_PlayerName.text = userInfo.m_sName;

            if (rewardsUI.m_PlayerPhoto != null)
            {
                CNameCardInfo card = m_DataCenterNet.GetNameCardInfo(userInfo.m_sDeivceId);
                if (card != null && card.GetPhoto() != null)
                    rewardsUI.m_PlayerPhoto.mainTexture = card.GetPhoto();
            }

            if (rewardsUI.m_PlayerTitle != null)
            {
                rewardsUI.m_PlayerTitle.text = string.Empty;
                if (m_GameData.m_TitleCenter != null)
                {
                    var title = m_GameData.m_TitleCenter.Get(userInfo.m_nTitle);
                    if (title != null)
                        rewardsUI.m_PlayerTitle.text = title.sName;
                }
            }

            if (rewardsUI.m_AdmireCount != null)
                rewardsUI.m_AdmireCount.Value = userInfo.m_nBeadmireCount;

            if (rewardsUI.m_BattleRate != null)
                rewardsUI.m_BattleRate.Value = userInfo.m_nCombatRatings;

            if (rewardsUI.m_Gold != null)
                rewardsUI.m_Gold.Value = userInfo.m_nGainGoldInGame;

            if (rewardsUI.m_Crystal != null)
                rewardsUI.m_Crystal.Value = userInfo.m_nGainCrystalInGame;


            if (rewardsUI.m_CharExp != null)
            {
                float barValue = 0f;
                float fCurRate = 0f;
                int lstLevel = 1, curLevel = 1;
                var charInfo = m_GameData.GetCharacterInfo(userInfo.m_nCharID);
                if (charInfo != null)
                {
                    lstLevel = userInfo.m_nCharLvl;
                    barValue = charInfo.GetExpRate(userInfo.m_nCharExp, lstLevel);
                    curLevel = userInfo.m_nCharLvlResult;
                    fCurRate = charInfo.GetExpRate(userInfo.m_nCharExpResult, curLevel);
                }
                rewardsUI.m_CharExp.Level = lstLevel;
                rewardsUI.m_CharExp.BarValue = barValue;
                rewardsUI.m_CharExp.SetAnimation(fCurRate, curLevel);
            }

            if (rewardsUI.m_HunterExp != null)
            {
                float barValue = 0f;
                float fCurRate = 0f;
                int lstLevel = 1, curLevel = 1;
                if (m_GameData.m_HunterCenter != null)
                {
                    lstLevel = userInfo.m_nHunterLvl;
                    barValue = m_GameData.m_HunterCenter.GetExpRate(userInfo.m_nHunterExp, lstLevel + 1);
                    curLevel = userInfo.m_nHunterLvlResult;
                    fCurRate = m_GameData.m_HunterCenter.GetExpRate(userInfo.m_nHunterExpResult, curLevel + 1);
                }
                rewardsUI.m_HunterExp.Level = lstLevel;
                rewardsUI.m_HunterExp.BarValue = barValue;
                rewardsUI.m_HunterExp.SetAnimation(fCurRate, curLevel);
            }

            var playerObj = GetPlayer(userInfo.m_nUID);
            rewardsUI.m_PlayerDeathFlag.enabled = playerObj != null && playerObj.isDead;

            rewardsUI.Show(true);

            if (userInfo == CGameNetManager.GetInstance().GetNetUserInfo())
            {
                rewardsUI.m_Admire.Enable = false;
            }
        }
    }
