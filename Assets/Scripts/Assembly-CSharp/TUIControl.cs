...
public class TUIControl : MonoBehaviour
{
    ...
    public virtual bool HandleInput(TUIInput input)
    {
        ...
        var controls = new List<TUIControl>(childCount);
        ...
        for (int i = 0; i < childCount; i++)
        {
            ...
            var control = child.GetComponent<TUIControl>();

            if (control != null && control.enabled) controls.Add(control);
        }

        if (controls.Count == 0) return false;

        controls.Sort(CompareControl);

        foreach (var control in controls) 
			if (control.HandleInput(input)) return true;
