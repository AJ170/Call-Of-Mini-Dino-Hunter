...
public class HttpClient
{
	...
	private class HttpTask
	{
		...
		public HttpTask(int taskId, string server, string url, float timeout, string key, string action, string request, string gameObjectName, string componentName, string functionName, string param)
		{
			...
			LogMgr.Log("requestdata = " + m_request);
		...
		public void Start()
		{
			try
			{
				...
				LogMgr.Log(m_url + "?action=" + m_action + " ....  requestdata length = " + m_requestData.Length);
			}
			catch
			{
				Complete(-1);
		...
		public bool Run()
		{
			...
			{
				LogMgr.Log("--responseHeaders--");
				foreach (KeyValuePair<string, string> responseHeader in m_www.responseHeaders)
				{
					LogMgr.Log(responseHeader.Key + " " + responseHeader.Value);
				}
				LogMgr.Log("-----");
				m_complete = true;
				if (m_www.error != null)
	...
	ServerInfo spoofedServerInfo = new ServerInfo("", "", -1, "");
	public ServerInfo GetServer(string server)
	{
		return spoofedServerInfo;
		/*if (!m_serverInfoMap.ContainsKey(server))
		{
			return null;
		}
	...
	{
		//ServerX.instance.SendHttp(server, action, data, gameObjectName, componentName, functionName, param);
		return 0;
