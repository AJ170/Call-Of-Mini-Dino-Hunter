...
public class CGameNetManager : MonoBehaviour
{
    ...
    //protected string m_sUserName = "TestAccount_" + UnityEngine.Random.Range(0, 10000000);
    protected string m_sUserName = "TestAccount_";
    ...
    {
        // fix
        m_sUserName += UnityEngine.Random.Range(0, 10000000).ToString();
    }
    ...
    public void UploadNameCard(string userId, string nickname, int title, CNCPack ncpack)
    {
        ...
        try
        {
            ...
            ServerX.instance.SendRequest(iMacroDefine.ServerUrl_CoopData, -1f, null, "userHandler.saveUser", JsonMapper.ToJson(hashtable));
        }
        catch (Exception ex)
        {
    ...
    public void UploadRankData(string userId, string nickname, int combatpower, int totalexp, int hunterlvl, int gold, int crystal, int applause)
    {
        ...
        ServerX.instance.SendRequest(iMacroDefine.ServerUrl_CoopData, -1f, null, "userHandler.insertLeaderboard", JsonMapper.ToJson(hashtable));
    }
    ...
    public void FetchNameCard(string sId, iServerHttp.OnSuccess onsuccess, iServerHttp.OnFailed onfail)
    {
        ...
        ServerX.instance.SendRequest(iMacroDefine.ServerUrl_CoopData, -1f, null, "userHandler.loadUser", JsonMapper.ToJson(hashtable), onsuccess, onfail);
    }
    ...
    public void FetchNameCards(string[] arrId, iServerHttp.OnSuccess onsuccess, iServerHttp.OnFailed onfail)
    {
        if (arrId != null && arrId.Length >= 1)
        {
            ...
            ServerX.instance.SendRequest(iMacroDefine.ServerUrl_CoopData, -1f, null, "userHandler.listUsers", JsonMapper.ToJson(hashtable), onsuccess, onfail);
        }
    }
    ...
    public void FetchRank(string sId, int nHunterExpTotal, iServerHttp.OnSuccess onsuccess, iServerHttp.OnFailed onfail)
    {
        ...
        ServerX.instance.SendRequest(iMacroDefine.ServerUrl_CoopData, -1f, null, "userHandler.listLeaderboard", JsonMapper.ToJson(hashtable), onsuccess, onfail);
    }
    ...
    public void FetchRankByUsers(string[] arrId, iServerHttp.OnSuccess onsuccess, iServerHttp.OnFailed onfail)
    {
        ...
        //Hashtable hashtable = new Hashtable();
        //hashtable.Add("userIds", text);
		ServerX.instance.SendRequest(iMacroDefine.ServerUrl_CoopData, -1f, null, "userHandler.listFriendLeaderboard", text, onsuccess, onfail);
    }
    ...
    public void FetchPhoto(string sDeviceId)
    {
        ...
        if (server != null)
        {
            ...
            ServerX.instance.SendRequest(server.url, server.timeout, server.key, "multiCsHandler.enter", JsonMapper.ToJson(hashtable), OnFetchGCAccount_S);
        }
    }
    ...
    {
		/*LogMgr.Log("gc friends ready to fetch uuid " + gcaccount);
        HttpClient.ServerInfo server = HttpClient.Instance().GetServer("dataserver");
        if (server != null)
        {
            //Hashtable hashtable = new Hashtable();
            ...
            //ServerX.instance.SendRequest(server.url, server.timeout, server.key, "multiCsHandler.enter", JsonMapper.ToJson(hashtable), OnFetchUUID_S);
            //ServerX.instance.SendRequest(server.url, server.timeout, server.key, "multiCsHandler.enter", iServerSaveData.GetInstance().CurDeviceId, OnFetchUUID_S);
        }*/
	}
        ...
        try
        {
            ...
            if (@string == "0")
            {
                ...
                if (string4.Length > 0 && m_ltFetchUUIDList != null && m_nFetchUUIDIndex >= 0 && m_nFetchUUIDIndex < m_ltFetchUUIDList.Count && iServerSaveData.GetInstance().CurDeviceId != string4)
                {
                    ...
                    LogMgr.Log(m_ltFetchUUIDList[m_nFetchUUIDIndex].m_sGCAccount + " = " + string4);
                }
            }
        }
        catch
